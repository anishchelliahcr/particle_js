...
function animate()
{
    ...
    for(var i=0;i<circlearray.length;i++)
    {
        ...
        for (var j = 0; j < circlearray.length; j++)
            if (Math.sqrt(Math.pow(circlearray[i].x - circlearray[j].x, 2) + Math.pow(circlearray[i].y - circlearray[j].y, 2)) <= Math.sqrt(Math.sqrt(larger*larger+smaller*smaller)/4)*32 && i != j)
            {
                c.beginPath()
